...
// Constants
// ==========================================
const MATE_SCORE: f64 = 10000.0;
const MATE_BOUND: f64 = 9000.0;
...
#[inline]
fn score_from_tt(mut score: f64, ply: u32) -> f64 {
    if score > MATE_BOUND { score -= ply as f64; }
    else if score < -MATE_BOUND { score += ply as f64; }
    score
}

#[inline]
fn score_to_tt(mut score: f64, ply: u32) -> f64 {
    if score > MATE_BOUND { score += ply as f64; }
    else if score < -MATE_BOUND { score -= ply as f64; }
    score
}
...
fn format_score(score: f64) -> String {
    if score > MATE_BOUND {
        format!("mate {}", ((MATE_SCORE - score) / 2.0).ceil())
    } else if score < -MATE_BOUND {
        format!("mate -{}", ((MATE_SCORE + score) / 2.0).ceil())
    } else {
        format!("cp {}", score as i32)
    }
}
    ...
    pub fn think(&mut self, abort: Arc<AtomicBool>) -> Option<Move> {
        ...
        for i in 0..count {
            ...
            if is_move_legal(&test_board, us, enemy) {
                root_moves.push(RootMove { m, score: score_move(m, None) as f64 });
            }
        }
...
pub fn alpha_beta(
    board: &Board, mut alpha: f64, mut beta: f64, depth: u32, ply: u32,
    abort: &Arc<AtomicBool>, nodes: &mut u64, tt: &mut TranspositionTable,
    zobrist: &ZobristKeys, history: &mut Vec<u64>,
) -> f64 {
    if check_time_abort(nodes, abort) { return 0.0; }

    if ply >= MAX_PLY {
        println!("info string Reached ply {}, aborting search.", ply);
    ...
    for i in 0..count {
        ...
        if abort.load(Ordering::Relaxed) { return 0.0; }
    ...
    if legal_moves == 0 {
        best_score = if gives_check(board, enemy, us) { -MATE_SCORE + (ply as f64) } else { 0.0 };
...
#[derive(Copy, Clone)]
struct RootMove { m: Move, score: f64 }
...
impl TranspositionTable {
    pub fn new(size_mb: usize) -> Self {
        ...
        Self {
            entries: vec![TTEntry { key: 0, score: 0.0, depth: 0, flag: TTFlag::Exact, best_move: None }; capacity],
            mask: capacity - 1,
        }
    }
...
pub fn quiescence_search(board: &Board, mut alpha: f64, beta: f64, abort: &Arc<AtomicBool>, nodes: &mut u64) -> f64 {
    if check_time_abort(nodes, abort) { return 0.0; }
